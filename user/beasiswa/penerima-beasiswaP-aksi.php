...
if ($aksi == 'tetapkan') {
			...
			$qr = '';
			for ($i=1; $i < $max; $i++) { 
				$qr .=" AND NOT id_bp='".$id_bp[$i]."'";
			}

			$sql = $con->prepare("UPDATE beasiswa_prestasi SET status_verifikasi='2' WHERE NOT id_bp='$id_bp[0]' $qr");
			$sql->execute(); // Eksekusi/Jalankan query


			$qu = $con->prepare("UPDATE periode SET penetapan=(penetapan+1) WHERE periode='$periode'");
			$qu->execute();
