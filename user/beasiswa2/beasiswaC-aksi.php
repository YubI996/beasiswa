...
$aksi = $_POST['aksi'];
			...
			// Ambil data yang dikirim dari form
				...
				$suratP1 = "file_berkas_cs/".$suratP;
				$suratAK1 = "file_berkas_cs/".$suratAK;
				$suratPN1 = "file_berkas_cs/".$suratPN;
				$propPN11 = "file_berkas_cs/".$propPN1;
				$propPN21 = "file_berkas_cs/".$propPN2;
				$suratTA1 = "file_berkas_cs/".$suratTA;
				$khs1 = "file_berkas_cs/".$khs;
				$suratK1 = "file_berkas_cs/".$suratK;
				$suratB1 = "file_berkas_cs/".$suratB;
				$ktm1 = "file_berkas_cs/".$ktm;
				$ktp1 = "file_berkas_cs/".$ktp;
				$akta1 = "file_berkas_cs/".$akta;
				$kk1 = "file_berkas_cs/".$kk;
				$domisili1 = "file_berkas_cs/".$domisili;
				$suratN1 = "file_berkas_cs/".$suratN;
				$burek1 = "file_berkas_cs/".$burek;
				$ijazah11 = "file_berkas_cs/".$ijazah1;
				$ijazah21 = "file_berkas_cs/".$ijazah2;
				$sertifikat11 = "file_berkas_cs/".$sertifikat1;
				$sertifikat21 = "file_berkas_cs/".$sertifikat2;
				$sertifikat31 = "file_berkas_cs/".$sertifikat3;
			...
			// Ambil data yang dikirim dari form
				...
				$d = $q->fetch();
				 ...
				 if (!empty($_FILES["suratP"]["tmp_name"]) && $SP == "") {
								$suratP = 'SP'.date("dHis").'-'.$_FILES['suratP']['name'];
								if(is_file("file_berkas_cs/".$d['surat_permohonan'])) 
								unlink("file_berkas_cs/".$d['surat_permohonan']); 
				 }else{
								$suratP = $d['surat_permohonan'];
				 }
				 if (!empty($_FILES["suratAK"]["tmp_name"]) && $SAK == "") {
								$suratAK = 'AK'.date("dHis").'-'.$_FILES['suratAK']['name'];
								if(is_file("file_berkas_cs/".$d['aktif_kuliah'])) 
								unlink("file_berkas_cs/".$d['aktif_kuliah']); 
				 }else{
								$suratAK = $d['aktif_kuliah'];
				 }
				 if (!empty($_FILES["suratPN"]["tmp_name"]) && $SPN == "") {
								$suratPN = 'SPN'.date("dHis").'-'.$_FILES['suratPN']['name'];
								if(is_file("file_berkas_cs/".$d['surat_penelitian'])) 
								unlink("file_berkas_cs/".$d['surat_penelitian']); 
				 }else{
								$suratPN = $d['surat_penelitian'];
				 }
				 if (!empty($_FILES["propPN1"]["tmp_name"]) && $PR1 == "") {
								$propPN1 = 'PR1'.date("dHis").'-'.$_FILES['propPN1']['name'];
								if(is_file("file_berkas_cs/".$d['proposal_penelitian1'])) 
								unlink("file_berkas_cs/".$d['proposal_penelitian1']); 
				 }else{
								$propPN1 = $d['proposal_penelitian1'];
				 }
				 if (!empty($_FILES["propPN2"]["tmp_name"]) && $PR2 == "") {
								$propPN2 = 'PR2'.date("dHis").'-'.$_FILES['propPN2']['name'];
								if(is_file("file_berkas_cs/".$d['proposal_penelitian2'])) 
								unlink("file_berkas_cs/".$d['proposal_penelitian2']); 
				 }else{
								$propPN2 = $d['proposal_penelitian2'];
				 }
				 if (!empty($_FILES["suratTA"]["tmp_name"]) && $STA == "") {
								$suratTA = 'STA'.date("dHis").'-'.$_FILES['suratTA']['name'];
								if(is_file("file_berkas_cs/".$d['surat_ta'])) 
								unlink("file_berkas_cs/".$d['surat_ta']); 
				 }else{
								$suratTA = $d['surat_ta'];
				 }
				 if (!empty($_FILES["khs"]["tmp_name"]) && $KHS == "") {
								$khs = 'KHS'.date("dHis").'-'.$_FILES['khs']['name'];
								if(is_file("file_berkas_cs/".$d['transkrip'])) 
								unlink("file_berkas_cs/".$d['transkrip']); 
				 }else{
								$khs = $d['transkrip'];
				 }
				 if (!empty($_FILES["suratK"]["tmp_name"]) && $SK == "") {
								$suratK = 'SK'.date("dHis").'-'.$_FILES['suratK']['name'];
								if(is_file("file_berkas_cs/".$d['non_pekerja'])) 
								unlink("file_berkas_cs/".$d['non_pekerja']); 
				 }else{
								$suratK = $d['non_pekerja'];
				 }
				 if (!empty($_FILES["suratB"]["tmp_name"]) && $SB == "") {
								$suratB = 'SB'.date("dHis").'-'.$_FILES['suratB']['name'];
								if(is_file("file_berkas_cs/".$d['non_beasiswa'])) 
								unlink("file_berkas_cs/".$d['non_beasiswa']); 
				 }else{
								$suratB = $d['non_beasiswa'];
				 }
				 if (!empty($_FILES["ktm"]["tmp_name"]) && $KTM == "") {
								$ktm = 'KTM'.date("dHis").'-'.$_FILES['ktm']['name'];
								if(is_file("file_berkas_cs/".$d['ktm'])) 
								unlink("file_berkas_cs/".$d['ktm']); 
				 }else{
								$ktm = $d['ktm'];
				 }
				 if (!empty($_FILES["ktp"]["tmp_name"]) && $KTP == "") {
								$ktp = 'KTP'.date("dHis").'-'.$_FILES['ktp']['name'];
								if(is_file("file_berkas_cs/".$d['ktp'])) 
								unlink("file_berkas_cs/".$d['ktp']); 
				 }else{
								$ktp = $d['ktp'];
				 }
				 if (!empty($_FILES["akta"]["tmp_name"]) && $AKT == "") {
								$akta = 'AKT'.date("dHis").'-'.$_FILES['akta']['name'];
								if(is_file("file_berkas_cs/".$d['akta_kelahiran'])) 
								unlink("file_berkas_cs/".$d['akta_kelahiran']); 
				 }else{
								$akta = $d['akta_kelahiran'];
				 }
				 if (!empty($_FILES["kk"]["tmp_name"]) && $KK == "") {
								$kk = 'KK'.date("dHis").'-'.$_FILES['kk']['name'];
								if(is_file("file_berkas_cs/".$d['kk'])) 
								unlink("file_berkas_cs/".$d['kk']); 
				 }else{
								$kk = $d['kk'];
				 }
				 if (!empty($_FILES["domisili"]["tmp_name"]) && $DOM == "") {
								$domisili = 'DOM'.date("dHis").'-'.$_FILES['domisili']['name'];
								if(is_file("file_berkas_cs/".$d['domisili'])) 
								unlink("file_berkas_cs/".$d['domisili']); 
				 }else{
								$domisili = $d['domisili'];
				 }
				 if (!empty($_FILES["suratN"]["tmp_name"]) && $SN == "") {
								$suratN = 'SN'.date("dHis").'-'.$_FILES['suratN']['name'];
								if(is_file("file_berkas_cs/".$d['non_narkoba'])) 
								unlink("file_berkas_cs/".$d['non_narkoba']); 
				 }else{
								$suratN = $d['non_narkoba'];
				 }
				 if (!empty($_FILES["burek"]["tmp_name"]) && $BR == "") {
								$burek = 'BR'.date("dHis").'-'.$_FILES['burek']['name'];
								if(is_file("file_berkas_cs/".$d['buku_rekening'])) 
								unlink("file_berkas_cs/".$d['buku_rekening']); 
				 }else{
								$burek = $d['buku_rekening'];
				 }
				 if (!empty($_FILES["ijazah1"]["tmp_name"]) && $IJZ1 == "") {
								$ijazah1 = 'IJZA'.date("dHis").'-'.$_FILES['ijazah1']['name'];
								if(is_file("file_berkas_cs/".$d['ijazah_sekolah'])) 
								unlink("file_berkas_cs/".$d['ijazah_sekolah']); 
				 }else{
								$ijazah1 = $d['ijazah_sekolah'];
				 }
				 if (!empty($_FILES["ijazah2"]["tmp_name"]) && $IJZ2 == "") {
								$ijazah2 = 'IJZB'.date("dHis").'-'.$_FILES['ijazah2']['name'];
								if(is_file("file_berkas_cs/".$d['ijazah_ta'])) 
								unlink("file_berkas_cs/".$d['ijazah_pt']); 
				 }else{
								$ijazah2 = $d['ijazah_pt'];
				 }
				 if (!empty($_FILES["sertifikat1"]["tmp_name"]) && $S1 == "") {
								$sertifikat1 = 'S1'.date("dHis").'-'.$_FILES['sertifikat1']['name'];
								if(is_file("file_berkas_cs/".$d['sertifikat1'])) 
								unlink("file_berkas_cs/".$d['sertifikat1']); 
				 }else{
								$sertifikat1 = $d['sertifikat1'];
				 }
				 if (!empty($_FILES["sertifikat2"]["tmp_name"]) && $S2 == "") {
								$sertifikat2 = 'S2'.date("dHis").'-'.$_FILES['sertifikat2']['name'];
								if(is_file("file_berkas_cs/".$d['sertifikat2'])) 
								unlink("file_berkas_cs/".$d['sertifikat2']); 
				 }else{
								$sertifikat2 = $d['sertifikat2'];
				 }
				 if (!empty($_FILES["sertifikat3"]["tmp_name"]) && $S3 == "") {
								$sertifikat3 = 'S3'.date("dHis").'-'.$_FILES['sertifikat3']['name'];
								if(is_file("file_berkas_cs/".$d['sertifikat3'])) 
								unlink("file_berkas_cs/".$d['sertifikat3']); 
				 }else{
...
if ($aksi == "del") {
			...
			if(is_file("file_berkas_cs/".$d['ijazah_pt'])) 
			unlink("file_berkas_cs/".$d['ijazah_pt']); 
