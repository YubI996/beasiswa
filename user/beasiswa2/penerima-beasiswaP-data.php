...
?>
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" style="margin-right:0px;">
                                    ...
                                    $periode = $dq['periode'];
                                    //    if ($periode == "") {
                                            $sql = $con->prepare("SELECT b.*, m.nama_mahasiswa, m.foto_mahasiswa, m.daerah, m.ilmu, m.jenjang FROM beasiswa_prestasi b, mahasiswa m WHERE b.id_mahasiswa=m.id_mahasiswa AND b.status_verifikasi='3' AND b.periode='$periode' ORDER BY b.ipk DESC");
                                    ...
                                    //    }
                                        ...
                                        while ($d=$sql->fetch()) {
                                            ...
                                            echo '
                                                <tr>
                                                    <td align="center"></td>
                                                    <td align="center"><a class="badge bg-green viewfile" data-toggle="modal" id="view-'.$no.'" data-no="'.$no.'" onclick="viewfile('.$d['id_bp'].')" data-target="#form-file1" style="word-wrap: break-word; word-break: break-all; white-space: normal;">'.$d['nama_mahasiswa'].'</a></td>
...
function viewfile(id){
            ...
            $("#btn-prev1").attr('class', 'btn bg-grey waves-effect');
                    ...
                    $.ajax({
                        url: 'penerima-beasiswaP-aksi.php', 
                        type: 'POST', 
                        data: data,  
                        processData: false,
                        contentType: false,
