...
?>
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" style="margin-right:0px;">
                                    ...
                                    //$periode = @$_GET['prd'];
                                    //    if ($periode == "") {
                                            $sql = $con->prepare("SELECT b.*, m.nama_mahasiswa, m.foto_mahasiswa, m.daerah, m.ilmu, m.jenjang FROM beasiswa_ta b, mahasiswa m WHERE b.id_mahasiswa=m.id_mahasiswa AND periode='$dpr' ORDER BY FIELD(b.status_verifikasi,'3','0','1','2'), b.ipk DESC, b.tgl_permohonan ASC");
                                    ...
                                    //    }
                                        ...
                                        if ($dpr == $d['periode']) { //jika periodenya sama 
                                            ...
                                            echo '
                                                <tr '.$colb.'>
                                                    <td align="center"></td>
                                                    <td align="center"><a class="badge '.$dc.' viewfile" data-toggle="modal" data-backdrop="static" data-keyboard="false" id="view-'.$no.'" data-no="'.$no.'" onclick="viewfile('.$d['id_bta'].')" data-target="#form-file1" style="word-wrap: break-word; word-break: break-all; white-space: normal;">'.$d['nama_mahasiswa'].'</a></td>
                                                    ...
                                                    <td align="center">
                                                    <a class="badge bg-teal btnedit" data-id="'.$no.'" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#form-modal" onclick="edit('.$d['id_bta'].');">
...
<?php  
    $sql = $con->prepare("SELECT * FROM kuotaku WHERE kategori='beasiswa_ta'");
    $sql->execute();

    while ($d = $sql->fetch()) {
        echo '<input type="text" id="'.$d['jenjang'].'_'.$d['daerah'].'" data-kipa="'.$d['kuota_ipa'].'" data-kips="'.$d['kuota_ips'].'" data-kdefault="'.$d['kuota_default'].'" data-iipa="'.$d['ipk_ipa'].'" data-iips="'.$d['ipk_ips'].'" data-idefault="'.$d['ipk_default'].'" style="display: none">';
    }
?>
...
var t = $('#example').DataTable( {
    ...
    t.on( 'order.dt search.dt', function () {
        ...
        t.column(6, {search:'applied', order:'applied'}).nodes().each( function (cell, i) { 

            var daerah = $('#col2_filter').val();
            var bidang = $('#col3_filter').val();
            var jenjang = $('#col4_filter').val();

            var rulek1 = $('#'+jenjang+'_'+daerah).data('kipa');
            var rulek2 = $('#'+jenjang+'_'+daerah).data('kips');
            var rulek3 = $('#'+jenjang+'_'+daerah).data('kdefault');

            var rulei1 = $('#'+jenjang+'_'+daerah).data('iipa');
            var rulei2 = $('#'+jenjang+'_'+daerah).data('iips');
            var rulei3 = $('#'+jenjang+'_'+daerah).data('idefault');
            ...
            var ipk = $(cell).text().trim(); 
            if ((i+1) <= kuota && parseFloat(ipk) >= minipk) {
                $(cell).parent().css('background-color', '#cedcf5');          
            }else{
                $(cell).parent().css('background-color', '#ffc7c1')
            }
