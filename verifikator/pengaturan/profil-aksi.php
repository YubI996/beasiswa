...
if ($aksi == "edt") {
					...
					$path = $host1."/user/master/foto_user/".$fotoBaru;

					if ($foto > 0 || $foto != "") {
							if(move_uploaded_file($tmp, $path)){ // Jika proses upload sukses
								if ($d['foto_user'] != "default.png") {
									if(is_file($host1."/user/master/foto_user/".$d['foto_user'])) // Jika foto ada
									unlink($host1."/user/master/foto_user/".$d['foto_user']); // Hapus file foto sebelumnya yang ada di folder foto
								}
							}
