...
if ($aksi == "verifying") {
			// Ambil data yang dikirim dari form
				...
				if ($statusV == 1) {
					...
					$q = $con->prepare("SELECT b.id_mahasiswa, m.nama_mahasiswa, m.email FROM mahasiswa m, beasiswa_ta b WHERE b.id_mahasiswa=m.id_mahasiswa AND b.id_bta='$id_bta'");
					$q->execute();
					...
					$statusP = 0;
                            ...
                            $sql = $con->prepare("UPDATE beasiswa_ta SET status_verifikasi=:statusV, status_perbaikan=:statusP, tgl_verifikasi=:tgl, keterangan=:keterangan, verifikator=:verifikator WHERE id_bta=:id_bta");
