...
function errorClear(idFL, classFl, idpesan){
        ...
        $("#btn-simpan").click(function(){ // Ketika tombol simpan di klik
            ...
            $.ajax({
                url: 'berkasTA-K-aksi.php', // File tujuan
        ...
        $(".viewfile1").click(function(){  
            ...
            if(filedata === 'jpg' || filedata === 'jpeg' || filedata === 'png' || filedata === 'gif'){
                var vf = '<i class="material-icons md-18">cloud_download</i> Download File : <a href="https://e-beasiswa.bontangkota.go.id/user/beasiswa/file_berkas_ta/'+ data1+'" download>'+data1+'</a><hr><img src="https://e-beasiswa.bontangkota.go.id/user/beasiswa/file_berkas_ta/'+ data1 +'" style="height:auto; width:100%; ">'; //jika berekstensi gambar tampilkan sebagai gambar
                document.getElementById('view-file').innerHTML = vf;
            }
            else if(filedata === 'pdf'){ //jika berekstensi pdf tampilkan sebagai file pdf
                document.getElementById('view-file').innerHTML = '<i class="material-icons md-48">cloud_download</i><br> Download File : <a href="https://e-beasiswa.bontangkota.go.id/user/beasiswa/file_berkas_ta/'+ data1+'" download>'+data1+'</a>';
                var vf = 'https://e-beasiswa.bontangkota.go.id/inc/assets/plugins/pdfjs/web/viewer.html?file=https://e-beasiswa.bontangkota.go.id/user/beasiswa/file_berkas_ta/'+ data1;
                window.open(vf);
            }else{
